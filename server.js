...
public/instructorregistericensed under the Apache License, Version 2.0 (the "License");
...
var adminHtml = fs.readFileSync(
  path.join(__dirname, "static/admin.html"),
  "utf8"
);

// var selecteduserHtml = fs.readFileSync(
//   path.join(__dirname, "static/selecteduser.html"),
//   "utf8"
// );

var instructorHtml = fs.readFileSync(
  path.join(__dirname, "static/instructor.html"),
  "utf8"
);
...
app.use(auth.getPassportinstructors().initialize());
...
app.get("/public/providers", (req, res) => {
    ...
    providers.push({ name: "ADFS SAML", url: "/public/provider/saml" });
  {
    ...
    } else if ("localUsersPath" in config) {
      providers.push({ name: "Local", url: "/public/provider/local" });
    }
  }
...
app.post("/puplic/instructorregister", auth.registerinstructor);
//this one is an authenticated request because is under /api
app.post("/api/localUser/updateUser", auth.updateLocalUser);
app.post("/api/localinstructor/updateinstructor", auth.updateLocalinstructor);

app.get(
  "/public/google",
  auth.getPassportinstructors().authenticate("google", {
    scope: [
...
// path for slack auth
app.get("/public/slack", auth.getPassportinstructors().authenticate("slack"));

// OAuth callback url
app.get(
  "/public/slack/callback",
  auth.getPassportinstructors().authenticate("slack", {
    successRedirect: "/main",
    failureRedirect: "/public/authFail.html",
  })
);
...
app.post(
  ...
  auth.getPassportinstructors().authenticate("saml", {
    successRedirect: "/main",
    failureRedirect: "/public/authFail.html",
  })
);
...
app.post(
  ...
  [
    ...
    function (req, res, next) {
      ...
      passportObj.authenticate("local", {
        failureRedirect: "/public/authFail.html",
      })(req, res, next);
    },
  ],
...
app.post(
  ...
  auth
    ...
    .authenticate("ldapauth", { failureRedirect: "/public/authFail.html" }),
  function (req, res) {
    res.redirect("/main");
  }
);
...
app.get("/instructor/users", async (req, res) => {
  let users = await db.fetchstudents(req.user.id);
  // console.log(users);
  res.send(users);
});

var selectedchallenges;
app.post("/selectedchallenges", function (req, res) {
  var selectedUser = req.body.accountId.substring("Local_".length);
...
var y = "";
app.post("/selecteduserChallange", function (req, res) {
  selecteduser = req.body.accountId;
  y = auth.retreivechallenges(selecteduser);
  res.send(y);
});

app.post("/stopplaying", function (req, res) {
  var selectedstopedchallenges = req.body.accountId;
  stopplayying = auth.retreivechallengestostop(selectedstopedchallenges);

  res.send(stopplayying);
});
...
app.get("/public/badge/:code", async (req, res) => {
  ...
  // Sanitize the req.url value before including it in the HTML response
  const safeUrl = DOMPurify.sanitize(req.url);
  // Replace the BADGE_URL placeholder in the HTML with the sanitized URL
  html = html.replace(/BADGE_URL/g, config.dojoUrl + safeUrl);
  res.send(html);
});
...
app.get("/instructor", (req, res) => {
  let updatedinstructorHtml = auth.addCsrfToken(req, instructorHtml);
  res.send(updatedinstructorHtml);
});
...
app.get("/challenges/solutions/:challengeId", (req, res) => {
  ...
  let hidden_challenges = y;
  // console.log("name", hidden_challenges.indexOf(challengeId));
  if (hidden_challenges && hidden_challenges.indexOf(challengeId) !== -1) {
    res.send("You are not allowed to view the solution");
  } else {
    // console.log("show");
