...
exports.insertUser = function (user, errCb, doneCb) {
  var con = getConn();
  var sql =
    "INSERT INTO users (id, accountId, teamId, familyName, givenName,code,choice) VALUES (null, ?, ?, ?, ?, ?, ? )";
...
//Creates a instructor in the database
exports.insertinstructor = function (user, errCb, doneCb) {
  var con = getConn();
  var sql =
    "INSERT INTO instructors (id, accountId, teamId, familyName, givenName,code, choice) VALUES (null, ?, ?, ?, ?, ?, ?)";
...
//fetches a instructor from the database
exports.getinstructor = function (accountId, errCb, doneCb) {
  var con = getConn();
  var sql = "SELECT * FROM instructors WHERE accountId = ? ";
  con.query(sql, [accountId], function (err, result) {
    if (err) handleErr(errCb, err);
    else {
      if (result.length >= 1) handleDone(doneCb, result[0]);
...
//fetches a instructor from the database by id
exports.getinstructorById = async (userId) => {
  var con = getConn();
  var sql = "SELECT * FROM instructors WHERE id = ? ";
  let user = await con.queryPromise(sql, [userId]);
  return user;
};
...
exports.deleteinstructor = function (accountId, errCb, doneCb) {
  var con = getConn();
  var sql = "DELETE FROM instructors WHERE accountId = ? ";
  con.query(sql, [accountId], function (err, result) {
    if (err) handleErr(errCb, err);
    else handleDone(doneCb, result);
  });
...
exports.updateUser = function (user, errCb, doneCb) {
  var con = getConn();
  var sql =
    "UPDATE users SET accountId = ?, teamId = ?, familyName = ?, givenName = ? , code = ? WHERE id = ?";
  con.query(
    sql,
    [
      user.accountId,
...
//updates the properties of a instructor  in the database
exports.updateinstructor = function (user, errCb, doneCb) {
  var con = getConn();
  var sql =
    "UPDATE instructors SET accountId = ?, teamId = ?, familyName = ?, givenName = ?, code = ? WHERE id = ?";
  con.query(
    sql,
    [
      user.accountId,
...
//fetches the list of instructors from the database only with public info
exports.fetchinstructors = function (errCb, doneCb) {
  var con = getConn();
  var sql = "SELECT givenName,familyName,teamId FROM instructors";
  con.query(sql, function (err, result) {
    if (err) handleErr(errCb, err);
    else {
      handleDone(doneCb, result);
...
exports.fetchstudents = async (userId) => {
  let con = getConn();
  
  let codeQuery = "SELECT code FROM instructors WHERE id = ?";
  let coderesult = await con.queryPromise(codeQuery, [userId]);
  // console.log(coderesult);
  let code = coderesult[0].code;
  // console.log(code);
  let usersQuery = "SELECT * FROM users WHERE code = ?";
  let users = await con.queryPromise(usersQuery, [code]);
  return users;
};
